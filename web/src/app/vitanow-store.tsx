...
import { avancement, enSommeil, joursDepuis } from "@/domain/vitanow";
import {
  CHALLENGES,
  CURRENT_STUDENT,
  CURRENT_STUDENT_ID,
  ...
  PROJECTS,
  THREADS,
} from "@/data/vitanow-corpus";

/**
 * vitanow-store.tsx — l'état applicatif de la démonstration.
 *
...
interface VitanowApi extends VitanowState {
  /* Lectures dérivées */
  myProjects: Project[];
  dormant: Project[];
...
const VitanowContext = createContext<VitanowApi | null>(null);

let compteur = 0;
const nouvelId = (prefixe: string) => `${prefixe}-${Date.now().toString(36)}-${compteur++}`;

export function VitanowProvider({ children }: { children: ReactNode }) {
  const [state, setState] = useState<VitanowState>(() => ({
    projects: PROJECTS,
    journal: JOURNAL,
    threads: THREADS,
  ...
  return <VitanowContext.Provider value={value}>{children}</VitanowContext.Provider>;
}

export function useVitanow(): VitanowApi {
  const ctx = useContext(VitanowContext);
  if (!ctx) throw new Error("useVitanow doit être utilisé dans un VitanowProvider");
  return ctx;
}
